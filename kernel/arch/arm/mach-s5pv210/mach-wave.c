...
static void panel_cfg_gpio(struct platform_device *pdev)
{
	...
	/* DISPLAY_HSYNC */
	s3c_gpio_cfgpin(GPIO_DISPLAY_HSYNC, S3C_GPIO_SFN(2));
	s3c_gpio_setpull(GPIO_DISPLAY_HSYNC, S3C_GPIO_PULL_NONE);

	/* DISPLAY_VSYNC */
	s3c_gpio_cfgpin(GPIO_DISPLAY_VSYNC, S3C_GPIO_SFN(2));
	s3c_gpio_setpull(GPIO_DISPLAY_VSYNC, S3C_GPIO_PULL_NONE);

	/* DISPLAY_DE */
	s3c_gpio_cfgpin(GPIO_DISPLAY_DE, S3C_GPIO_SFN(2));
	s3c_gpio_setpull(GPIO_DISPLAY_DE, S3C_GPIO_PULL_NONE);

	/* DISPLAY_PCLK */
	s3c_gpio_cfgpin(GPIO_DISPLAY_PCLK, S3C_GPIO_SFN(2));
...
void lcd_cfg_gpio_early_suspend(void)
{
	...
	for (i = 0; i < 24; i++)	
	{
		...
		gpio_set_value(GPIO_LCD_D0 + i, 0);
	}
...
#define DISPLAY_CS	S5PV210_MP05(1)
...
//for S8530 - LG4573
static struct spi_board_info spi_board_info_lg4573[] __initdata = {
	{
		.modalias	= "lg4573",
//		.platform_data	= &wave_panel_data,
		.max_speed_hz	= 1200000,
...
//This for ALL display type S8500 and S8530
...
static void __init qt_touch_init(void)
{
    ...
    gpio = GPIO_TOUCH_EN;                     /* XMMC3DATA_3 */
    gpio_request(gpio, "TOUCH_EN");
    s3c_gpio_cfgpin(gpio, S3C_GPIO_OUTPUT);
    gpio_direction_output(gpio, 1);
    gpio_free(gpio);

    gpio = GPIO_TOUCH_INT;                             /* XMSMADDR_5 */
...
/*
	if(gp2a_get_proximity_enable()){
		s3c_gpio_slp_cfgpin(S5PV210_GPJ1(4),S3C_GPIO_SLP_OUT1); 			//Proximity sensor
		s3c_gpio_slp_setpull_updown(S5PV210_GPJ1(4), S3C_GPIO_PULL_NONE);
	}*/

	if (gpio_get_value(GPIO_PS_ON)) {
		s3c_gpio_slp_setpull_updown(GPIO_AP_SDA_28V, S3C_GPIO_PULL_NONE);
		s3c_gpio_slp_setpull_updown(GPIO_AP_SCL_28V, S3C_GPIO_PULL_NONE);
	} else {
...
static void __init wave_map_io(void)
{
	...
	s5p_reserve_bootmem(wave_media_devs, ARRAY_SIZE(wave_media_devs));
#ifdef CONFIG_MTD_ONENAND
	s5pc110_device_onenand.name = "s5pc110-onenand";
#endif
}

unsigned int pm_debug_scratchpad = 0; //unset in Wave for now.
		...
		struct meminfo *mi)
{
	...
	mi->bank[1].size = 255 * SZ_1M; //1Meg for ram console and debug scratchpad
	mi->bank[1].node = 1;
...
static void __init wave_machine_init(void)
{
	...
	s3cfb_set_platdata(&tl2796_data);
        ...
        spi_register_board_info(spi_board_info_lg4573, ARRAY_SIZE(spi_board_info));
...
void s3c_setup_keypad_cfg_gpio(int rows, int columns)
{
	...
	/* Set GPH3 pins to EINT */
       	s3c_gpio_cfgpin(S5PV210_GPH3(0), S3C_GPIO_SFN(3));
	...
	s3c_gpio_cfgpin(S5PV210_GPH3(2), S3C_GPIO_SFN(3));
        ...
        s3c_gpio_cfgpin(S5PV210_GPH2(0), S3C_GPIO_SFN(3));
        s3c_gpio_setpull(S5PV210_GPH2(0), S3C_GPIO_PULL_NONE);

	s3c_gpio_cfgpin(S5PV210_GPH2(1), S3C_GPIO_SFN(3));
